...
export async function inviteFriendById(user, friend) {
    try {
        const isFriendAlready = await databases.listDocuments(
            ...
            [Query.equal('accountId1', user.accountId), Query.equal('accountId2', friend.accountId)]
        )
        ...
        const res2 = await databases.createDocument(
            ...
            ID.unique(),
            {
                accountId1: friend.accountId,
                accountId2: user.accountId,
...
export async function createGame(form) {
    try {
        ...
        console.log("uploadedFile: ", thumbnailUrl);
